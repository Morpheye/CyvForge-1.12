...
public class GuiUtils extends Gui {
    ...
    public static int drawString(String text, int x, int y, int color, boolean shadow) {
        ...
        if (lines.length > 1) {
            ...
            for (int i = 0; i < lines.length; i++)
                j += mc.fontRenderer.drawString(lines[i], x, (y + i * (mc.fontRenderer.FONT_HEIGHT + 2)), color, shadow);
            return j;
        }
        return mc.fontRenderer.drawString(text, x, y, color, shadow);
    }
    ...
    public static int drawScaledString(String text, int x, int y, boolean shadow, int color, float scale) {
        GlStateManager.pushMatrix();
        GlStateManager.scale(scale, scale, 1.0F);
        int i = drawString(text, (int)(x / scale), (int)(y / scale), color, shadow);
        GlStateManager.scale(Math.pow(scale, -1.0D), Math.pow(scale, -1.0D), 1.0D);
        GlStateManager.popMatrix();
        return i;
    }
    ...
    public static void drawChromaString(String textIn, int xIn, int y, boolean shadow) {
        ...
        for (int j = 0; j < (textIn.split("\n")).length; j++) {
            ...
            for (char c : textIn.split("\n")[j].toCharArray()) {
                ...
                drawString(String.valueOf(c), x, y + j * (mc.fontRenderer.FONT_HEIGHT + 2), color, shadow);
    ...
    public static void drawStringInLimitedArea(String str, int x, int x2, int y, boolean shadow, int color) {
        ...
        for(String s : originalString) {
            length += mc.fontRenderer.getStringWidth(s + " ");
            if(x + length > x2) {
                finalString.append("\n");
                length = 0;
            }
        ...
        for(String s : finalString.toString().split("\n")) {
            ...
            offset += mc.fontRenderer.FONT_HEIGHT + 3;
        }
    }

    public static int drawCenteredString(String text, int x, int y) {
        return drawString(text, x - mc.fontRenderer.getStringWidth(text) / 2, y);
    }

    public static int drawCenteredString(String text, int x, int y, int color) {
        return drawString(text, x - mc.fontRenderer.getStringWidth(text) / 2, y, color);
    }

    public static int drawCenteredString(String text, int x, int y, int color, boolean shadow) {
        return drawString(text, x - mc.fontRenderer.getStringWidth(text) / 2, y, color, shadow);
    }

    public static int drawScaledCenteredString(String text, int x, int y, boolean shadow, float scale) {
        return drawScaledString(text, x - (int)(mc.fontRenderer.getStringWidth(text)*scale / 2), y, shadow, scale);
    }

    public static int drawScaledCenteredString(String text, int x, int y, boolean shadow, int color, float scale) {
        return drawScaledString(text, x - (int)(mc.fontRenderer.getStringWidth(text)*scale / 2), y, shadow, color, scale);
    }

    //Textured rectangle methods
    ...
    //Untextured rectangle methods

    public static void drawRectOutline(int x, int y, int x2, int y2, int color) {
        drawHorizontalLineS(x, x2, y, color);
        drawHorizontalLineS(x, x2, y2, color);
        drawVerticalLineS(x, y2, y, color);
        drawVerticalLineS(x2, y2, y, color);
    }
